function [ R, subgrad, data ] = risk_par_mv_b( data, W )
...
%          fprintf('\n');
        ...
        for i = 1 : numel(LP_NFfeatures)
            if data.precomputeLossTables
                [r, psi] = loss_par_mv_b_flandmark(data.options, LP_NFfeatures{i}, LP_Yaws{i}, LP_GTs{i}, LP_Lgt{i}, LP_psi_gt{i}, data.model, data.featuresPool, data.flandmark_pool, W);
            else
                [r, psi] = loss_par_mv_b_flandmark(data.options, LP_NFfeatures{i}, LP_Yaws{i}, LP_GTs{i}, LP_kappas(i), LP_psi_gt{i}, data.model, data.featuresPool, data.flandmark_pool, W);
            end;
            subgrad = subgrad + psi;
            R = R + r;
        end;
